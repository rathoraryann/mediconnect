    ...
    <form onSubmit={onSubmitHandler} className='min-h-[80vh] flex items-center'>
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg'>
        ...
        <p>Please {state == 'Sign Up' ? "sign up" : "login"} to book appointment</p>
        {
          ...
          <p>Full name</p>
          <input className='border border-zinc-600 rounded w-full p-2 mt-1' type="text" name="" id="" onChange={(e)=>setName(e.target.value)} value={name}/>
        </div>
        }
        <div className='w-full'>
          <p>Email</p>
          <input className='border border-zinc-600 rounded w-full p-2 mt-1'  type="text" name="" id="" onChange={(e)=>setEmail(e.target.value)} value={email}/>
        </div>
        <div className='w-full'>
          <p>Password</p>
